...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Risky\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=558898368395:+55 88998368395\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nSalve, mano! Bem-vindo ao grupo *${mdata.subject}*\n\nEspero que goste do grupo ❤️`
				...
				teks = `Tchau @${num.split('@')[0]} O corno saiu 😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD RISKY BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] RISKY? Este é um recurso especial para o RISKY ❌',
					ownerB: '[❗] RISKY? Este é um recurso especial para o RISKY ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de RISKY. Registre-se para fazer amizade com o bot RISKY por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar RISKY|17\n\n──「 RISKY BOT 」──`,
			...
			const ownerNumber = ["558898368395@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"558898368395@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["558898368395@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["558898368395@s.whatsapp.net"]//ubah nomor lo
			const premium = ["558898368395@s.whatsapp.net","558591221977@s.whatsapp.net","558798062862@s.whatsapp.net","552288206653@s.whatsapp.net"] 
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("bot")){
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
				...
				if (messagesC.includes("RISKY")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/RISKYf.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
			...
			client.updatePresence(from, Presence.composing)
			data = fs.readFileSync('./src/riskyjokes.js');
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS RISKY BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @RISKY domina vida
          ...
          reply('Espere..')
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o RISKY ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT RISKY ✨* 」\n*│+ wa.me/5547992091566*╰──────*「 *RISKY* 」*────*\n\n*_SE QUER SER ADMIN DO BOT RISKY_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/558898368395/*╰──────*「 * RISKY* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO RISKY BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'canal':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/zlRP3m2b.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do RISKY:*\n\n https://www.youtube.com/channel/UCyLZqVdhMzVqRYplY5JXGyg'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* RISKY\n*YOUTUBE:* https://www.youtube.com/channel/UCyLZqVdhMzVqRYplY5JXGyg\n*WPP:* wa.me/+5588998368395\n*INSTA:* @daviikaua.69\n\n\nEspero que tenham gostado do bot 🐊🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
        ...
        case 'promover':
				...
				case 'bemvindo':
					...
					if (Number(args[0]) === 1) {
						...
						reply('1 para ativar, 0 para desativar, lerdão vc em mano')
					}
                                      break
				case 'clonar':
					if (!isGroup) return reply(mess.only.group)
